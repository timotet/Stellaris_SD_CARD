 ...
 * web site. It was modified to work with a Stellaris EK-LM4F120H5QR (stellaris launchpad)
 * evaluation board.
 */
...
/* Peripheral definitions for stellaris launchpad board */
// SSI port
...
#define SDC_GPIOD_SYSCTL_PERIPH  SYSCTL_PERIPH_GPIOD
#define SDC_SSI_CLK              GPIO_PIN_0
#define SDC_SSI_RX               GPIO_PIN_2
#define SDC_SSI_TX               GPIO_PIN_3
#define SDC_SSI_PINS             (SDC_SSI_TX | SDC_SSI_RX | SDC_SSI_CLK)

//port PA6 for card chip enable line
#define SDC_GPIOA_SYSCTL_PERIPH  SYSCTL_PERIPH_GPIOA
#define SDC_SSI_CE_BASE          GPIO_PORTA_BASE
#define SDC_SSI_CE               GPIO_PIN_6                // PA6

// asserts the CE pin to the card active low
static
void SELECT (void)
{
    GPIOPinWrite(SDC_SSI_CE_BASE, SDC_SSI_CE, 0x00);
}

// de-asserts the CS pin to the card inactive high
static
void DESELECT (void)
{
    GPIOPinWrite(SDC_SSI_CE_BASE, SDC_SSI_CE, SDC_SSI_CE);
}
...
BYTE rcvr_spi (void)
{
    ...
    SSIDataGet(SDC_SSI_BASE, &rcvdat); /* read data from rx fifo */
    //SSIDataGetNonBlocking(SDC_SSI_BASE, &rcvdat);
...
void send_initial_clock_train(void)
{
    ...
    GPIOPinTypeSSI(SDC_GPIO_PORT_BASE, SDC_SSI_TX | SDC_SSI_CLK);
...
void power_on (void)
{
    ...
    SysCtlPeripheralEnable(SDC_GPIOD_SYSCTL_PERIPH);
    SysCtlPeripheralEnable(SDC_GPIOA_SYSCTL_PERIPH);

    // enable SDC_SSI_CE as an output
    GPIOPinTypeGPIOOutput(SDC_SSI_CE_BASE, SDC_SSI_CE);

    // Configure the pin alternate function for SSI3 on pins PD0, PD3, and PD4.
    GPIOPinConfigure(GPIO_PD0_SSI3CLK);
    GPIOPinConfigure(GPIO_PD2_SSI3RX);
    GPIOPinConfigure(GPIO_PD3_SSI3TX);

    /*
     * Configure the appropriate pins to be SSI instead of GPIO. The SDC_SSI_CE
    ...
    GPIOPadConfigSet(SDC_GPIO_PORT_BASE, SDC_SSI_CLK | SDC_SSI_TX, GPIO_STRENGTH_4MA, GPIO_PIN_TYPE_STD);
    GPIOPadConfigSet(SDC_SSI_CE_BASE, SDC_SSI_CE, GPIO_STRENGTH_4MA, GPIO_PIN_TYPE_STD);



    /* Configure the SSI2 port */
    SSIConfigSetExpClk(SDC_SSI_BASE, 80000000, SSI_FRF_MOTO_MODE_0, SSI_MODE_MASTER, 400000, 8);
